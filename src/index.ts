...
const BI_DIRECTIONAL_LINKS_PATTERN = /\[\[([^|\]\n]+)(?:\|([^\]\n]+))?\]\]/y;
...
export const BiDirectionalLinks: (options: BiDirectionalLinksOptions) => PluginSimple = ({
  ...
  return (md) => {
    md.inline.ruler.after('text', RULE_NAME, (state) => {
      BI_DIRECTIONAL_LINKS_PATTERN.lastIndex = state.pos;
      const matched = BI_DIRECTIONAL_LINKS_PATTERN.exec(state.src);
